    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.goodId) {
            this.$http({
              url: this.$http.adornUrl(`/mall/indexbanner/info/${this.dataForm.goodId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.imgUrl = data.indexBanner.imgUrl
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/mall/indexbanner/${!this.dataForm.goodId ? 'save' : 'update'}`),
              method: 'post',
